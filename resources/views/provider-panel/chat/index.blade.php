                    ...
                    <div class="contacts-outter">
                        <ul class="list-unstyled contacts">
                            @foreach ($chats as $chat)
                                <li data-toggle="tab" data-target="#inbox-message-{{ $chat->id }}" class="active">
                                    <img alt="" class="img-circle medium-image" src="https://bootdey.com/img/Content/avatar/avatar1.png">

                                    <div class="vcentered info-combo">
                                        <h3 class="no-margin-bottom name"> {{ $chat->user->name }} </h3>
                                    </div>
                ...
                <div id="sent" class="contacts-outter-wrapper tab-pane">
                    ...
                    <div class="contacts-outter">
                        <ul class="list-unstyled contacts success">
                            @foreach ($chats as $chat)
                                <li data-toggle="tab" data-target="#sent-message-{{ $chat->id }}">
                                    <img alt="" class="img-circle medium-image" src="https://bootdey.com/img/Content/avatar/avatar1.png">
                    ...
                    <div class="message-chat">
                        <div class="chat-body">
                            ...
                            @foreach ($chat->messages as  $message)
                                <div class="message info   <?php if($message->sender_id != 'guest') echo "my-message"; ?>">
                                    <img alt="" class="img-circle medium-image" src="https://bootdey.com/img/Content/avatar/avatar1.png">
                        ...
                        <div class="chat-footer">
                            ...
                            <button type="button" class="send-message-button btn-info sendRequestBtn"  value="{{ $chat->user_id }}"> <i class="fa-regular fa-paper-plane"></i></button>
                        </div>
                    </div>
                </div>
            @endforeach
...
.contacts-outter::-webkit-scrollbar {
  display: none;
}
...
.panel-search-form.success.form-group.has-feedback.no-margin-bottom {
    display: none
}
...
<script>
    $('.sendRequestBtn').on('click', function() {
        let userId = $(this).val();          // Replace with the actual user ID
        let providerId = {{ auth()->user()->id }};     // Replace with the actual provider ID
        let message = $(this).parent().find(".send-message-text").val() ?? "Message"; // Replace with the actual message content
        let button = $(this);
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    let newMessage = `<div class="message info">
                                    <img alt="" class="img-circle medium-image" src="https://bootdey.com/img/Content/avatar/avatar1.png">
...
`;
                    ...
                    button.parent().parent().find('.chat-body').animate({
                        scrollTop: 5000
                    }, 100); // 1000ms is the scroll duration; adjust as needed
